...
FEATURE_SET_NAMES_TO_ANGULARITY = {
    "canonical": [False, False, False, True, True, True, False, False, False],  # Dists, dihedrals, bond angles
    "canonical-full-angles": [True, True, True, False, False, False],  # Only dihedrals are periodic
    "canonical-minimal-angles": [True, True, True, False],  # Only dihedrals are periodic
...
class CathCanonicalAnglesDataset(Dataset):
    ...
    feature_is_angular = {
        "angles": [False, False, False, True, True, True, False, False, False],  # Dists, dihedrals, bond angles
    ...
    def __getitem__(
        ...
        # Note: angles is a pandas DataFrame at this point
        if self.means is not None and not ignore_zero_center:
            ...
            # Subtract mean - this should keep it as a DataFrame
            angles = angles - self.means

            # Only wrap periodic dihedral angles (phi, psi, omega), NOT bond angles
            # Use the proper feature_is_angular mapping for the full dataset
            full_feature_is_angular = CathCanonicalAnglesDataset.feature_is_angular["angles"]
            angular_idx = np.where(full_feature_is_angular)[0]
            # Get column names for angular features
            angular_cols = [angles.columns[i] for i in angular_idx]
            # Apply modulo wrapping to angular columns
            angles[angular_cols] = utils.modulo_with_wrapped_range(
                angles[angular_cols].values, -np.pi, np.pi
            )

        # Subset angles to ones we are actually using as features
        # Convert to numpy array after subsetting
        angles = angles.loc[
...
class CathCanonicalAnglesOnlyDataset(CathCanonicalAnglesDataset):
    ...
    feature_is_angular = {"angles": [True, True, True, False, False, False]}  # Only dihedrals are periodic
...
class CathCanonicalMinimalAnglesDataset(CathCanonicalAnglesOnlyDataset):
    ...
    * Non-dihedrals: tau
    ...
    feature_is_angular = {"angles": [True, True, True, False]}  # Only dihedrals are periodic
